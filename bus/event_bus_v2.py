...
import logging
import sys
sys.path.insert(0, '/root/.openclaw/workspace/agent/bus')

# v3.2: AsyncEventProcessor 集成
try:
    from async_event_processor import AsyncEventProcessor, EventBusAsyncWrapper
    ASYNC_AVAILABLE = True
except ImportError:
    ASYNC_AVAILABLE = False
    AsyncEventProcessor = None
    EventBusAsyncWrapper = None
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if "status" not in columns:
                logger.info("Migrating events table to v2.0 schema...")
            ...
            if "processing_by" not in columns:
            ...
            if "processed_at" not in columns:
                conn.execute("ALTER TABLE events ADD COLUMN processed_at REAL")
            conn.commit()
...
class EventBus:
    ...
    def __init__(self,
        ...
        if ASYNC_AVAILABLE and AsyncEventProcessor:
            self.async_processor = AsyncEventProcessor(self)
    ...
    def process_pending(self, max_events: int = 5) -> int:
        ...
        v3.2: process_pending 保持同步处理（稳定可靠）
        AsyncEventProcessor 作为独立后台服务运行
        返回处理的事件数
        """
        if not self.store:
            return 0
    ...
    def get_recent_events(self, limit: int = 20) -> List[Dict]:
        """
        v2.8: 获取最近的事件（用于CognitionLoop缓存更新）
        
        返回最近处理过的事件列表（包含pending、processing、done）
        """
        if not self.store:
            return []
